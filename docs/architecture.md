...
Each box is a persistent host dir, `~/containers/<name>`, bind-mounted as `/home/dev`. On first boot the entrypoint copies the image's skeleton (`/home/dev.skel`) into the mounted dir; subsequent boots preserve existing files and pick up new ones from image rebuilds. Per-box state (Claude creds, gh/git auth) persists at `~/containers/<name>/.dev-state` where `CLAUDE_CONFIG_DIR` points. The launcher uses `--userns=keep-id` and mounts the knowledge library read-only only if the submodule is present.

The interface is `dev <name>`; binding an arbitrary host dir is gone by design. The Podman machine is provisioned by `dev --vm-init` to mount only `~/containers`, `~/.dotfiles/private/knowledge`, and `~/.claude/memory`, never the home root. So host secrets are structurally unreachable from every container, including ones spawned through the host-root-equivalent podman socket. That socket stays opt-in (`dev --sock on` or `DEV_DOCKER_SOCK=1`); with the secret-blind machine it lets a box spawn sibling containers reachable on the host via published ports. This is the fix for the [#191](decisions/0001-secret-blind-dev-vm.md) container escape; see [docs/decisions/0001-secret-blind-dev-vm.md](decisions/0001-secret-blind-dev-vm.md) for the threat model and alternatives. The `dev-container` host is a standalone home config for user `dev` with `opsPackages.enableGui = false` and `enableHeavy = false`, and ships an `hm-switch` script to re-apply the home config in a running container without an image rebuild.
