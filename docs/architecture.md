...
Each box is a persistent host dir, `~/containers/<name>`, mounted as the container's home at the same path (so compose volume paths resolve). Repos live under `~/containers/<name>/repos`; per-box state (Claude creds, gh/git auth) persists at `~/containers/<name>/.dev-state`, mounted at `/home/dev/.dev-state` where `CLAUDE_CONFIG_DIR` points. The launcher uses `--userns=keep-id` and mounts the knowledge library read-only only if the submodule is present.
